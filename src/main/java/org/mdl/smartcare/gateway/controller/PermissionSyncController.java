...
public class PermissionSyncController {
  ...
  @Autowired private PermissionSyncService permissionSyncService;

  /**
   * Sync permissions from database to Keycloak as realm roles This operation is idempotent - it
   * will create new roles or update existing ones
   *
    ...
    return permissionSyncService
        ...
        .map(
            response -> {
              ...
              HttpStatus status =
                  response.getFailureCount() > 0 ? HttpStatus.PARTIAL_CONTENT : HttpStatus.OK;
        ...
        .map(
            status -> {
              ...
              HttpStatus httpStatus =
                  status.isConnected() ? HttpStatus.OK : HttpStatus.SERVICE_UNAVAILABLE;
