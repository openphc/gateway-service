...
@Service
public class DatabasePermissionLoaderServiceImpl implements DatabasePermissionLoaderService {

  private static final Logger logger =
      LoggerFactory.getLogger(DatabasePermissionLoaderServiceImpl.class);
  ...
  @Override
  public Mono<Void> loadPermissionsFromDatabase() {
    ...
    return permissionRepository
        ...
        .doOnError(
            error -> {
              logger.error(
                  "Failed to load permissions from database. Using YAML configuration as fallback.",
                  error);
            })
        .onErrorResume(
            error -> {
  ...
  @Override
  public Map<String, List<PermissionConfig.PermissionRule>> convertToPermissionMappings(
